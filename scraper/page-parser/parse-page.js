...
let getHtmlColData = (rowsRule,colsRule,doc,i,prePageData)=>{
  ...
  for (let colRule of colsRule){
    ...
    if (colRule.rule_value_type == "attribute"){
      ...
      prePageData[colRule.name] = dataElement.text() ? dataElement.text().trim() : colRule.default_value;
    }
    // 设置录入数据库时查询数据是否重复的字段
...
let getAttr = (data, colRule, dataElement) =>{
  if (dataElement && dataElement.value()){
    data[colRule.name] = colRule.base_url ? getAbsUrl(colRule.base_url, dataElement.value().trim()) : dataElement.value().trim();
  }else{
    data[colRule.name] = ''
  }
}
